...
public class Solution5976 {
    ...
    public static void main(String[] args) {
        ...
        Solution5976 solution5976 = new Solution5976();
        boolean b = solution5976.checkValid(matrix);
    ...
    public boolean checkValid(int[][] matrix) {
        ...
        int[] counts_row = new int[len+1];
        int[] counts_column = new int[len+1];
        for (int i = 0; i < len; i++) {
            //行
            for (int j = 0; j < len; j++) {
                counts_row[matrix[i][j]]++;
            }
            //列
            for (int j = 0; j < len; j++) {
                counts_column[matrix[j][i]]++;
            }
            for (int j = 1; j <= len; j++) {
                if(counts_column[j]!=1||counts_row[j]!=1){
                    return  false;
                }
            }
